...
public class TerapieListAdapter extends RecyclerView.Adapter<TerapieListAdapter.TerapieViewHolder> {
    ...
    public class TerapieViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(DocumentSnapshot documentSnapshot, int position) {
            ...
            sveglieRef.whereEqualTo(KEY_FARMACO, documentSnapshot.getId()).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    ...
                    if(task.getResult().getDocuments().size() > 0) {
                        ...
                        for (int i = 0; i < task.getResult().getDocuments().size(); i++) {
                            messaggio.append("• I giorni ").append(Util.getGiorniSveglia((List<Boolean>) task.getResult().getDocuments().get(i).getData().get(KEY_SETTIMANA))).append(" alle ore ").append(Util.TimestampToOrario((Timestamp) task.getResult().getDocuments().get(i).getData().get(KEY_ORARIO)));
