...
public class TrasfusioniFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Button buttonAggiungiTrasfusione = root.findViewById(R.id.buttonAggiungiTrasfusione);
        buttonAggiungiTrasfusione.setOnClickListener(v -> Navigation.findNavController(root).navigate(R.id.action_trasfusioniFragment_to_aggiungiTrasfusioneFragment));
        ...
        trasfusioniRef.whereLessThan(KEY_DATA, Calendar.getInstance().getTime()).orderBy(KEY_DATA, Query.Direction.DESCENDING).limit(1).addSnapshotListener((value, error) -> {
            if(value.getDocuments().size() > 0){
                ...
                Button buttonCronologiatrasfusioni = root.findViewById(R.id.buttonCronologiatrasfusioni);
                buttonCronologiatrasfusioni.setVisibility(View.VISIBLE);
                buttonCronologiatrasfusioni.setOnClickListener(v -> Navigation.findNavController(root).navigate(R.id.action_trasfusioniFragment_to_cronologiaTrasfusioneFragment));
            }

            //Prendo la prossima trasfusione
            trasfusioniRef.whereGreaterThan(KEY_DATA, Calendar.getInstance().getTime()).limit(1).addSnapshotListener((value1, error1) -> {
                if(value1.getDocuments().size() > 0){
                    prossima = Util.TimestampToDate((Timestamp) value1.getDocuments().get(0).get(KEY_DATA));
