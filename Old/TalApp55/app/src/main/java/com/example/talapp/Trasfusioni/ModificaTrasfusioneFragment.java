...
public class ModificaTrasfusioneFragment extends Fragment {
    ...
    private void getTrasfusione(String id, TextInputEditText EThb, AutoCompleteTextView unita, TextInputEditText ETNote) {
        trasfusioniRef.document(id).addSnapshotListener((EventListener<DocumentSnapshot>) (value, error) -> {
            if(value != null) {
                trasfusione_old = value.getData();
                TextInputEditText TXVdata = root.findViewById(R.id.TXVDataTrasfusione);
                ...
                unita.setSelection(((ArrayAdapter) unita.getAdapter()).getPosition(trasfusione_old.get(KEY_UNITA)));
