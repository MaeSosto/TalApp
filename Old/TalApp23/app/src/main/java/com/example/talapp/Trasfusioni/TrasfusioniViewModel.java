   ...
   public void InsertTrasfusione(View root, Spinner Sunita, Calendar date, EditText ETNote) {
    ...
    public void getTrasfusioniGiorno(Calendar giorno, RecyclerView recyclerView){
        if(isConnectedToInternet(context)){
            ...
            trasfusioniRef//.whereEqualTo(KEY_TRASFUSIONE_DATA, var)
                    //.whereLessThan(KEY_TRASFUSIONE_DATA, DateToLong(fine.getTime())).whereGreaterThan(KEY_TRASFUSIONE_DATA, Util.DateToLong(inizio.getTime()))
                   // trasfusioniRef.whereEqualTo(KEY_TRASFUSIONE_DATA, giorno)
                    .addSnapshotListener(new EventListener<QuerySnapshot>() {
                        ...
                        public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                            ...
                            Calendar fine = giorno;
                            fine.set(Calendar.HOUR_OF_DAY, 24);
                            fine.set(Calendar.MINUTE, 59);
                            fine.set(Calendar.SECOND, 59);
                            fine.set(Calendar.MILLISECOND, 59);
                            Log.i("Inizio", String.valueOf(inizio));
                            for (int i = 0; i< mTrasfusioni.size(); i++){
                                if(((long) mTrasfusioni.get(i).get(KEY_TRASFUSIONE_DATA) >= DateToLong(inizio.getTime())) && ((long) mTrasfusioni.get(i).get(KEY_TRASFUSIONE_DATA) <= DateToLong(fine.getTime()))){
                                    mTrasfusioni.remove(i);
                                }
                            }
                            TrasfusioniListAdapter trasfusioniListAdapter = new TrasfusioniListAdapter(context);
                            recyclerView.setAdapter(trasfusioniListAdapter);
