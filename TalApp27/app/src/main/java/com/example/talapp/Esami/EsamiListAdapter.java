...
import static com.example.talapp.Utils.Util.KEY_ESAME_REFERTO;
...
public class EsamiListAdapter extends RecyclerView.Adapter<EsamiListAdapter.EsamiViewHolder> {
    ...
    public class EsamiViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(DocumentSnapshot documentSnapshot, int position) {
            ...
            Timestamp tmp = (Timestamp) esame.get(KEY_ESAME_DATA);
            TXVgiorno.setText(esame.get(KEY_ESAME_NOME)+" del "+ Util.DateToString(tmp.toDate()) + " ore "+ DateToOrario(tmp.toDate()));

            TextView tipo = itemView.findViewById(R.id.TXVTipoEsame);
            tipo.setText("Tipo: "+ esame.get(KEY_ESAME_TIPO));

            TextView esito = itemView.findViewById(R.id.TXVEsitoEsame);
            if(esame.containsKey(KEY_ESAME_ESITO)){
                esito.setText("Esito: "+ esame.get(KEY_ESAME_ESITO));
            }
            else{
            ...
            if(esame.containsKey(KEY_ESAME_REFERTO)){
            ...
            if(esame.containsKey(KEY_ESAME_DIGIUNO)){
            ...
            if(esame.containsKey(KEY_ESAME_ATTIVAZIONE)){
                attivazione.setText("Attivazione 24: SI");
                if(esame.containsKey(KEY_ESAME_RICORDA)){
                    ricorda.setVisibility(View.VISIBLE);
                    ricorda.setText("Ricorda di: " + esame.get(KEY_ESAME_RICORDA));
                }
            }
            else{
            ...
            if(esame.containsKey(KEY_ESAME_NOTE)){
                note.setVisibility(View.VISIBLE);
                note.setText("Note: " + esame.get(KEY_ESAME_NOTE));
            }
