...
public class EsamiViewModel extends AndroidViewModel {
    ...
    public void getAllEsami(RecyclerView recyclerView, String tipoEsami) {
        if(isConnectedToInternet(context)){
            esamiRef.whereEqualTo(KEY_ESAME_TIPO, tipoEsami)
                    ...
                    .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                            if (task.isSuccessful()) {
                                ...
                                Timestamp tmp = (Timestamp) esame_old.get(KEY_ESAME_DATA);
    ...
    public void setEsamiStatus(View root, String tipoEsami){
        Log.d("TAB", "setEsamiStatus");
        TextView TXVProssimiEsami = root.findViewById(R.id.TXVProssimiEsami);
        Button btnAggiungiDati = root.findViewById(R.id.buttonAggiungiUltimoEsame);
        RecyclerView recyclerView = root.findViewById(R.id.RecyclerViewProssimiEsami);
        Calendar calendar = Calendar.getInstance();
        Date oggi = calendar.getTime();

        if(isConnectedToInternet(context)){
            //Prendo l'ultimo esame
            esamiRef.whereLessThan(KEY_ESAME_DATA, oggi)
                    .whereEqualTo(KEY_ESAME_TIPO, tipoEsami)
                    .orderBy(KEY_ESAME_DATA, Query.Direction.DESCENDING)
                    .limit(1)
                    .addSnapshotListener(new EventListener<QuerySnapshot>() {
                        @Override
                        public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                            if(value != null){
                                Log.d("TAB", String.valueOf(value.getDocuments().get(0).get(KEY_ESAME_NOME)));
                                Timestamp tmp = (Timestamp) value.getDocuments().get(0).get(KEY_ESAME_DATA);
                                ultimo =  tmp.toDate();
//
                                //Bottone aggiungi dati ultimo esame
                            ...
                            //Prendo i prossimi esami
                            esamiRef.whereGreaterThan(KEY_ESAME_DATA, oggi)
                                    .whereEqualTo(KEY_ESAME_TIPO, tipoEsami)
                                    .limit(3)
                                    .addSnapshotListener(new EventListener<QuerySnapshot>() {
                                        @Override
                                        public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                                            if(value != null) {
                                                TXVProssimiEsami.setText("Prossimi esami");
                                                ...
                                                //AIUTO
                                                //TXVProssimiEsami.setText("Non ci sono prossimi esami "+ tipoEsami);
